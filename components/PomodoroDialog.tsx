...
export default function PomodoroDialog({ open, onClose }: Props) {
  ...
  // load/save settings and timer state
  useEffect(() => {
    ...
    if (raw) {
      try {
        ...
        if (parsed.running !== undefined) setRunning(parsed.running);
        if (parsed.completedRounds !== undefined) setCompletedRounds(parsed.completedRounds);
        if (parsed.lastUpdate && parsed.running) {
  ...
  useEffect(() => {
    // update timeLeft when settings or mode changes (only if not running)
    if (!running) {
  ...
  // Save all state including timer state
  useEffect(() => {
  ...
  }, [workMin, shortMin, longMin, roundsBeforeLong, autoStartNext, mode, timeLeft, running, completedRounds]);

  // timer loop - continues running even when dialog is closed
  useEffect(() => {
    if (!running) return;
  ...
  // Only reset settings view when dialog opens, don't reset timer
  useEffect(() => {
    if (!open) {
  ...
  const styles = {
    ...
    statusIndicator: {
      ...
      color: running ? '#10b981' : '#6b7280', fontSize: '0.875rem'
    }
  };
        ...
        {!showSettings && (
          <div style={styles.statusIndicator}>
            <div style={{
              ...
              backgroundColor: running ? '#10b981' : '#6b7280'
            }}></div>
            <span>Timer {running ? 'Running' : 'Paused'} - Continues when closed</span>
          </div>
        )}
        ...
        {!showSettings ? (
          ...
          <main style={styles.settingsSection}>
            ...
            <div style={styles.quickActions}>
              ...
              <button onClick={() => { localStorage.removeItem(STORAGE_KEY); alert('Settings cleared!'); window.location.reload(); }} style={styles.quickButton}>Clear Saved Settings</button>
            </div>
          </main>
        )}

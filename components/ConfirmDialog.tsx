...
interface ConfirmDialogProps {
  ...
  onClose: () => void;
  title: string;
  children: React.ReactNode; // بدل message
  confirmButtonText?: string;
  onConfirm?: () => void; // اختياري للـ todo dialog
...
const ConfirmDialog: React.FC<ConfirmDialogProps> = ({
  ...
  return (
    <div
      className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-50 flex justify-center items-start pt-24 p-4 transition-opacity duration-300"
      aria-modal="true"
      role="dialog"
      onClick={onClose}
    >
      ...
      >
        ...
        <div className="text-text-secondary mb-6 max-h-96 overflow-y-auto">{children}</div>

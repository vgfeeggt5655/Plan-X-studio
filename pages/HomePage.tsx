...
import { updateUser } from '../services/authService'; // ⬅️ مهم لحفظ todo
...
const parseTodoList = (todo: string | undefined | null): string[] => {
  if (!todo || typeof todo !== 'string' || todo.trim() === '') return [];
  try {
...
const HomePage: React.FC = () => {
  ...
  const { user, setUser } = useAuth(); // لازم نقدر نعدل بيانات اليوزر
  const navigate = useNavigate();

  const todoList = useMemo(() => parseTodoList(user?.todo_list), [user]);
  ...
  const handleAddTask = async () => {
    ...
    const updatedTasks = [...todoList, newTask.trim()];
    const updatedUser = { ...user, todo_list: JSON.stringify(updatedTasks) };
  ...
  return (
    <div className="space-y-12 pb-12">
      ...
      {/* ✅ Todo List Section */}
      <div className="container mx-auto px-4">
        <h2 className="text-2xl font-bold text-text-primary mb-4">Your Todo List</h2>
        <div className="bg-surface p-4 rounded-xl shadow-md border border-border-color">
          <ul className="space-y-2 mb-4">
            {todoList.map((task, index) => (
              <li key={index} className="flex justify-between items-center bg-background px-3 py-2 rounded-lg">
          ...
          <div className="flex gap-2">
            <input
              ...
              className="flex-grow px-3 py-2 rounded-lg border border-border-color focus:outline-none"
            />
            <button
              onClick={handleAddTask}
              className="px-4 py-2 bg-primary text-white rounded-lg hover:bg-cyan-400"
            >
              Add
            </button>
          </div>

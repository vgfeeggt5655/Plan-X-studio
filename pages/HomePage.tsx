...
const HomePage: React.FC = () => {
  ...
  // Check if prayer dialog should be shown (once per session)
  const shouldShowPrayerDialog = useCallback(() => {
    const sessionKey = 'prayerDialogShown_' + new Date().toDateString();
    const hasShownToday = sessionStorage.getItem(sessionKey);
    return !hasShownToday;
  }, []);

  // Mark prayer dialog as shown for this session
  const markPrayerDialogAsShown = useCallback(() => {
    const sessionKey = 'prayerDialogShown_' + new Date().toDateString();
  ...
  useEffect(() => {
    ...
    // Show prayer dialog after a short delay when page loads
    if (shouldShowPrayerDialog()) {
      const timer = setTimeout(() => {
        setShowPrayerDialog(true);
      }, 800); // Show after 800ms to allow page to load smoothly

      return () => clearTimeout(timer);
    }
  }, [fetchInitialData, shouldShowPrayerDialog]);

  const handlePrayerDialogClose = () => {
    setShowPrayerDialog(false);
    markPrayerDialogAsShown();
  };
  ...
  return (
    <div className="space-y-12 pb-12">
      ...
      {/* Prayer Dialog */}
      <StudyPrayerDialog
        isOpen={showPrayerDialog}
        onClose={handlePrayerDialogClose}
      />
